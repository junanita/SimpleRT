...
namespace CoreLib
{
	namespace WinForm
	{
		...
		MouseEventArgs::MouseEventArgs(WinForm::MouseButton btn, int _x, int _y, int _delta, unsigned int buttons)
		...
		KeyEventArgs::KeyEventArgs(wchar_t key, int & keycode, bool ctrl, bool shift, bool alt)
			:Key(key), KeyCode(keycode), BaseControl(ctrl), Shift(shift), Alt(alt)
		{
		}
	}
}